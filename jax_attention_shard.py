...
if __name__ == '__main__':
    ...
    #input = jax.device_put_replicated(input, devices)
    ...
    '''
    device = input.device_buffer.device()
    print(f'Variable is on device: {device}')
    for param in jax.tree_leaves(params_flash):
        device = param.device_buffer.device()
        print(f'Parameter is on device: {device}')
    '''
    #params_flash = flax.jax_utils.replicate(params_flash)

    for _ in range(5):
        flash_attention.apply(params_flash, input).block_until_ready()

    print(make_jaxpr(flash_attention.apply)(params_flash, input))
...
vanilla attention forward pass took 1.331070899963379 seconds

flash attention forward pass took 20.347038984298706 seconds

batch_size 32, seq_len: 1024, speed 0.07. times
